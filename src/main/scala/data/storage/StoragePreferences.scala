...
private def dataRef: URIO[StoragePreferences, SubscriptionRef[Elem]] =
  for pref ← storagePreferences
    yield pref.dataRef

private def dataStream: URIO[StoragePreferences, UStream[Elem]] =
  for ref ← dataRef
    yield ref.changes

private def data: URIO[StoragePreferences, Elem] =
  for
    ref ← dataRef
    elem ← ref.get
  yield elem
...
private def updateChannel(): URIO[StoragePreferences, Channel[Boolean]] =
  for pref ← storagePreferences
